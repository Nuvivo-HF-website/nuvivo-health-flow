...
SELECT 'General Consultation', 'Comprehensive health consultation and examination', 'consultation', 150.00, 30, true
WHERE NOT EXISTS (SELECT 1 FROM public.services WHERE name = 'General Consultation')
UNION ALL
SELECT 'Follow-up Consultation', 'Follow-up appointment for ongoing care', 'consultation', 100.00, 20, true
WHERE NOT EXISTS (SELECT 1 FROM public.services WHERE name = 'Follow-up Consultation')
UNION ALL
SELECT 'Health Check-up', 'Routine health screening and preventive care', 'screening', 200.00, 45, true
WHERE NOT EXISTS (SELECT 1 FROM public.services WHERE name = 'Health Check-up');
...
AND NOT EXISTS (
    ...
    WHERE ss.specialist_id = s.id AND ss.service_id = srv.id
);